...
std::vector<std::string> class_names{"Car","Van"};
float nms_iou_thresh = 0.01;
int pre_nms_top_n = 4096;
bool do_profile{false};
std::string model_path;
std::string engine_path = "../model/onnx.fp32.engine";
...
int main(int argc, char **argv)
{
  ...
  std::cout << "Loading Data Dir: " << data_path << std::endl;
  ...
  for (int i = 0; i < 66; i++) {
    ...
    std::string bin_file_name = dataFile.substr(0, dataFile.find_last_of('.'));
    std::string save_file_name = "../data/box/bbox_" + output_path + bin_file_name.substr(bin_file_name.find_last_of('/') + 1) + ".txt";
